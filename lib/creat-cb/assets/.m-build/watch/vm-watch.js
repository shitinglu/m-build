...
var vmWatch=(dir, config,done) => {
  ...
  var watcher = gulp.watch(src.temp + "*.vm");
  ...
  watcher.on('change',function(link, stats){
    ...
    return gulp.src(link)
      ...
      .pipe(print(function(){
          gutil.log(notice(`${link}已经编译 并被移动到${dist.css}${baseName}`));
          done();
      }));
  });
  ...
  watcher.on('add',function(link, stats){
    ...
    return gulp.src(link)
      .pipe(gulp.dest(dist.temp))
      .pipe(print(function(){
          gutil.log(notice(`${link}已经创建 并被移动到${dist.css}${baseName}`));
          done();
      }));
  });
  ...
  watcher.on('unlink',function(link, stats){
    var baseName=path.basename(link);
    return gulp.src(`${dist.temp}${baseName}`, {read: false})
      .pipe(gulpclean({force: true}))
      .pipe(print(function(filepath){
        gutil.log(notice(`${link} 和 ${dist.temp}${baseName} 已经被删除`));
        done();
      }));
  });
