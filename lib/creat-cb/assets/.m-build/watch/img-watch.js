...
var imgWatch = (dir, config, done) => {
  ...
  watcher.on('change', function(link, stats) {
    ...
    return gulp.src(link)
      .pipe(gulp.dest(dist.i+subcatalog))
      .pipe(print(function() {
        gutil.log(notice(`${link}已经添加 并被移动到${dist.i}${subcatalog.length>0?subcatalog+'/':subcatalog}${baseName}`));
        done();
      }));
  ...
  watcher.on('add', function(link, stats) {
    ...
    return gulp.src(link)
      .pipe(gulp.dest(dist.i+subcatalog))
      .pipe(print(function() {
        gutil.log(notice(`${link}已经创建 并被移动到${dist.i}${subcatalog.length>0?subcatalog+'/':subcatalog}${baseName}`));
        done();
      }));
