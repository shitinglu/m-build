...
var vmWatch=(dir, config,done) => {
  ...
  var watcher = gulp.watch(src.temp + "**/*.*");
  ...
  watcher.on('change',function(link, stats){
    ...
    return gulp.src(link)
      .pipe(gulp.dest(dist.temp+subcatalog))
      .pipe(print(function(){
          gutil.log(notice(`${link}已经编译 并被移动到${dist.temp}${subcatalog.length>0?subcatalog+'/':subcatalog}${baseName}`));
          done();
      }));
  });
  ...
  watcher.on('add',function(link, stats){
    ...
    return gulp.src(link)
      .pipe(gulp.dest(dist.temp+subcatalog))
      .pipe(print(function(){
          gutil.log(notice(`${link}已经创建 并被移动到${dist.temp}${subcatalog.length>0?subcatalog+'/':subcatalog}${baseName}`));
          done();
      }));
  });
